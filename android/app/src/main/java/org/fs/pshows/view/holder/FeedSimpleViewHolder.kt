...
import org.fs.pshows.common.glide.CustomPlaceholderTarget
...
class FeedSimpleViewHolder(view: View,
  private val glide: GlideRequests,
  private val imageBuilder: ImageBuilder,
  private val activeGenres: Variable<List<Genre>>,
  private val proxy: ShowDaoProxy): BaseFeedViewHolder(view) {
  ...
  constructor(parent: ViewGroup, glide: GlideRequests, imageBuilder: ImageBuilder, activeGenres: Variable<List<Genre>>, proxy: ShowDaoProxy): this(parent.inflate(R.layout.view_feed_item), glide, imageBuilder, activeGenres, proxy)

  override fun bind(value: Show) {
    this.show = value
    glide.clear(viewImagePoster)

    val path = value.posterPath ?: String.EMPTY
    val url = imageBuilder.createPosterUri(path = path)
    glide.load(url)
      .applyCrop()
      .into(CustomPlaceholderTarget(viewImagePoster))

    viewTextTitle.text =  value.name
    viewTextAverage.text = String.format(Locale.getDefault(), "%.2f", value.voteAverage)

    val genreIds: LongArray = value.genreIds ?: LongArray(0)
