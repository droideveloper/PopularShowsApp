...
class LoadDetailCreditIntent(
  private val showId: Long,
  private val detailRepository: DetailRepository): ObservableIntent<DetailFragmentModel>() {

  override fun invoke(): Observable<Reducer<DetailFragmentModel>> = detailRepository.credits(showId)
    .concatMap(::success)
    .onErrorResumeNext(::failure)
    .startWith(initial())
    .subscribeOn(Schedulers.io())

  private fun success(resource: Resource<List<Credit>>): Observable<Reducer<DetailFragmentModel>> = when(resource) {
    is Resource.Success<List<Credit>> -> Observable.just(
      { o -> o.copy(state = Operation(LOAD_DETAIL_CREDIT, false), data = resource.data ?: emptyList()) },
      { o -> o.copy(state = Idle, data = emptyList()) })
    is Resource.Failure<List<Credit>> -> Observable.just(
  ...
  private fun initial(): Reducer<DetailFragmentModel> = { o -> o.copy(state = Operation(LOAD_DETAIL_CREDIT, true), data = emptyList()) }
}