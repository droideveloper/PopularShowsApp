...
class LoadDetailImageIntent(
  private val showId: Long,
  private val detailRepository: DetailRepository): ObservableIntent<DetailFragmentModel>() {

  override fun invoke(): Observable<Reducer<DetailFragmentModel>> = detailRepository.images(showId)
    .concatMap(::success)
    .onErrorResumeNext(::failure)
    .startWith(initial())
    .subscribeOn(Schedulers.io())

  private fun success(resource: Resource<List<Image>>): Observable<Reducer<DetailFragmentModel>> = when(resource) {
    is Resource.Success<List<Image>> -> Observable.just(
      { o -> o.copy(state = Operation(LOAD_DETAIL_IMAGE, false), data = resource.data ?: emptyList()) },
      { o -> o.copy(state = Idle, data = emptyList()) })
    is Resource.Failure<List<Image>> -> Observable.just(
  ...
  private fun initial(): Reducer<DetailFragmentModel> = { o -> o.copy(state = Operation(LOAD_DETAIL_IMAGE, true), data = emptyList()) }
}