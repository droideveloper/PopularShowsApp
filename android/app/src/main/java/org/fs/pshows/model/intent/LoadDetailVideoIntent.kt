...
class LoadDetailVideoIntent(
  private val showId: Long,
  private val detailRepository: DetailRepository): ObservableIntent<DetailFragmentModel>() {

  override fun invoke(): Observable<Reducer<DetailFragmentModel>> = detailRepository.videos(showId)
    .concatMap(::success)
    .onErrorResumeNext(::failure)
    .startWith(initial())
    .subscribeOn(Schedulers.io())

  private fun success(resource: Resource<List<Video>>): Observable<Reducer<DetailFragmentModel>> = when(resource) {
    is Resource.Success<List<Video>> -> Observable.just(
      { o -> o.copy(state = Operation(LOAD_DETAIL_VIDEO, false), data = resource.data ?: emptyList()) },
      { o -> o.copy(state = Idle, data = emptyList()) })
    is Resource.Failure<List<Video>> -> Observable.just(
  ...
  private fun initial(): Reducer<DetailFragmentModel> = { o -> o.copy(state = Operation(LOAD_DETAIL_VIDEO, true), data = emptyList()) }
}